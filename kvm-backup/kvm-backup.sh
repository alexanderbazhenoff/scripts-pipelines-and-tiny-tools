    ...
    if [ $command_use = "--active" ]; then
        ...
        do
            ...
            if [ -f $activevm.snapshot ]; then
                echo "`date +"%Y-%m-%d_%H-%M-%S"` Snapshot $activevm.snapshot sucsesfully created" >> $logfile
                echo "`date +"%Y-%m-%d_%H-%M-%S"` Snapshot $activevm.snapshot sucsesfully created"
            else
                printf "`date +"%Y-%m-%d_%H-%M-%S"` WARNING! Snapshot wasn't created.\n" >> $logfile
                printf "`date +"%Y-%m-%d_%H-%M-%S"` There's no guaranty that resulting copy of VM may have consistent data.\n" >> $logfile
                printf "`date +"%Y-%m-%d_%H-%M-%S"` WARNING! Snapshot wasn't created.\n"
                printf "`date +"%Y-%m-%d_%H-%M-%S"` There's no guaranty that resulting copy of VM may have consistent data.\n"
            fi
            ...
            do
                ...
                if [[ $snap_path == "-" ]] || [[ $snap_path =~ \.iso$ ]] || [[ $snap_path == \.ISO$ ]]; then
                    ...
                    echo "`date +"%Y-%m-%d_%H-%M-%S"`$result_cmd" >> $logfile
                    echo "`date +"%Y-%m-%d_%H-%M-%S"`$result_cmd"
                    if [[ $snap_path =~ \.snapshot$ ]]; then
                        echo "`date +"%Y-%m-%d_%H-%M-%S"` Removing snapshot $snap_path" >> $logfile
                        echo "`date +"%Y-%m-%d_%H-%M-%S"` Removing snapshot $snap_path"
                        rm -f $snap_path
                    else
                        echo "`date +"%Y-%m-%d_%H-%M-%S"` $snap_path is not snapshot, skipping." >> $logfile
                        echo "`date +"%Y-%m-%d_%H-%M-%S"` Looks like you have copied images on the go or no single snapshot for this disk" >> $logfile
                        echo "`date +"%Y-%m-%d_%H-%M-%S"` $snap_path is not snapshot, skipping."
                        echo "`date +"%Y-%m-%d_%H-%M-%S"` Looks like you have copied images on the go or no single snapshot for this disk"
                    fi
                fi
            done
