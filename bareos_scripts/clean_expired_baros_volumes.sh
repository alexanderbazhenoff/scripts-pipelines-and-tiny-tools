...
process_volume() {
  ...
  if [[ $FLAGS_test == "no" ]]; then
    echo "$FLAGS_action volume=$filename$1 yes" | bconsole
    ...
    echo "(test mode): echo \"$FLAGS_action volume=$filename$1 yes\" | bconsole"
  fi
}
...
if [[ $FLAGS_action = "delete" ]] || [[ $FLAGS_action = "prune" ]] || [[ $FLAGS_action = "purge" ]]; then
  cd $poolpath || exit 1
  ...
  do
    ...
    if [[ $FLAGS_filter == "none" ]]; then
      echo "filter is none"
      process_volume "$filename"
    else
      # filter by volume status if $FLAGS_filter is not 'none'
      [[ -n $(echo "list volume" | bconsole | grep "$FLAGS_filter" | grep "$filename" | cut -d ' ' -f6) ]] && \
        process_volume "$filename"
    fi
  done
else
