...
Usage:
    python3 graph/traffic_item_statistics.py -i $PWD/Traffic\ Item\ Statistics1.csv \
...
if __name__ == '__main__':
    ...
    args_parser.add_argument('-t', '--test_name',
                             required=True, help='Test name')
    args_parser.add_argument('-m', '--item_name',
                             required=True, help='Traffic Item name')
    settings = args_parser.parse_args()

    if isfile(settings.inputfile) and basename(settings.inputfile) == 'Traffic Item Statistics.csv':
        ...
        layout = go.Layout(
            title="<b>%s </b>%s: Loss, Rx Frame Rate, Frames Delta and Store-Forward Avg Latency" % (settings.testname,
                                                                                                     settings.itemname),
            width=1900,
            height=930,
            xaxis=dict(
        ...
        print("Error, wrong input data: defined path is not a file or not 'Traffic Item Statistics.csv'")
